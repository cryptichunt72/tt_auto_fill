...
def generate():
    ...
    try:
		...
		# Build the full render context (supports upper-case placeholders)
		render_ctx = {**ctx, **{k.upper(): v for k, v in ctx.items()}}

		wanted  = list_template_vars(tpl_path)
		missing = [m for m in wanted if m not in render_ctx]   # or case-insensitive:
		# keyset = {k.upper() for k in render_ctx}
		# missing = [m for m in wanted if m.upper() not in keyset]

		if missing:
		    return {"ok": False, "error": "Missing variables for template",
		            "missing_variables": missing}, 400
        ...
        beneficiary_name = (beneficiary.get("beneficiary_name") or "Unknown").strip().replace("/", "-")
