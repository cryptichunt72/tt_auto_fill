...
from flask import Blueprint, render_template, request, jsonify, url_for, send_from_directory, abort
...
@invoice_bp.post("/api/suggest_items")
...
@invoice_bp.post("/api/suggest_items_text")
...
def build_template_prompt(style_instructions: str) -> str:
    return textwrap.dedent(f"""
        ...
        Use this semantic skeleton and class names (do not omit):
        <body>
          <div class="invoice">
        ...
        - Reflect the style recipe exactly (layout/typography/rhythm).
...
def ask_llm_for_jinja_template(style_instructions: str) -> str:
    ...
    resp = client.chat_completions.create(  # fallback for older SDKs; try chat.completions first
        model="gpt-4o-mini",
        temperature=0.0,
        messages=[
            {"role": "system", "content": "Return a single JSON object with keys 'name' and 'html'."},
        ...
        stream=False,
        messages=[
            {"role": "system", "content": "Return only a single JSON object with keys 'name' and 'html'. Nothing else."},
            {"role": "user", "content": prompt},
        ],
    )

    raw = (resp.choices[0].message.content if hasattr(resp.choices[0], "message") else resp.choices[0].delta.get("content")) or "{}"
    try:
...
@invoice_bp.post("/api/parse_paste")
def api_parse_paste():
...
@invoice_bp.post("/api/llm_extract")
def api_llm_extract():
    body = request.get_json(force=True)
...
@invoice_bp.post("/api/save_template")
...
@invoice_bp.get("/invoices/<company_slug>/<path:filename>")
